export const createBridgeCommandRegistry = ({
    ...
    // Edge-triggered polling state for @sprite.when_touching_object
    // Mirrors VM hat edgeActivated: fires when touching goes false -> true.
    const touchingPollers = new Map(); // key: `${targetId}:${name}` -> {prevTouching, timer}
    let touchingPollCounter = 0;

    const stopTouchingPoller = (key) => {
        const poller = touchingPollers.get(key);
        if (poller) {
            clearInterval(poller.timer);
            touchingPollers.delete(key);
        }
    };

    const startTouchingPoller = ({ runtime, target, name }) => {
        if (!target || typeof target.isTouchingObject !== "function") return;
        const key = `${target.id}:${name}`;
        if (touchingPollers.has(key)) return; // already polling
        const poller = {
            prevTouching: false,
            timer: null,
        };
        touchingPollers.set(key, poller);
        poller.timer = setInterval(() => {
            let touching = false;
            try {
                touching = Boolean(target.isTouchingObject(name));
            } catch (e) {
                // target may be disposed
            }
            if (touching && !poller.prevTouching && forwardEventToPython) {
                forwardEventToPython({
                    name: "touching_object",
                    value: name,
                });
            }
            poller.prevTouching = touching;
        }, 50);
    };
    ...
    return {
        ...
        touchingObject: {
            ...
            execute: async ({ runtime, target, args }) => {
                ...
                if (!name) return;
                // Also start VM hats so block-code projects react too.
                runtime.startHats("event_whentouchingobject", {
                    TOUCHINGOBJECTMENU: name,
                });
                startTouchingPoller({ runtime, target, name });
            },
        },
        createClone: {
            resolveTarget: true,
            ...
            execute: async ({ runtime, target, args }) => {
                ...
                if (
                    !cloneTarget ||
                    typeof cloneTarget.makeClone !== "function"
                ) {
                    return;
                }
        ...
        setCostume: {
            ...
            execute: async ({ target, args }) => {
                ...
                if (!applied) {
                    throw new Error(
                        `Costume not found: ${String(args[0] || "")}`,
                    );
                }
            },
        },
        setBackdrop: {
            ...
            execute: async ({ target, args }) => {
                ...
                if (!applied) {
                    throw new Error(
                        `Backdrop not found: ${String(args[0] || "")}`,
                    );
                }
            },
        },
        ...
        devicePinMode: {
            ...
            execute: async ({ runtime, args }) => {
                ...
                if (
                    !peripheral ||
                    typeof peripheral.setPinMode !== "function"
                ) {
                    throw new Error(
        ...
        deviceAnalogWrite: {
            ...
            execute: async ({ runtime, args }) => {
                ...
                if (
                    !peripheral ||
                    typeof peripheral.setPwmOutput !== "function"
                ) {
                    throw new Error(
                ...
                const clamped = Math.max(
                    0,
                    Math.min(255, Math.round(rawValue)),
                );
        ...
        deviceAnalogRead: {
            ...
            execute: async ({ runtime, args }) => {
                ...
                if (
                    !peripheral ||
                    typeof peripheral.readAnalogPin !== "function"
                ) {
                    throw new Error(
        ...
        deviceServoWrite: {
            ...
            execute: async ({ runtime, args }) => {
                ...
                const clamped = Math.max(
                    0,
                    Math.min(180, Math.round(rawAngle)),
                );
