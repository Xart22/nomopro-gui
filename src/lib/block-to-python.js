...
function blockLine(block, depth) {
    ...
    switch (type) {
        ...
        case "event_whentouchingobject": {
            const tgt = f("TOUCHINGOBJECTMENU") || "";
            if (tgt === "_mouse_") {
                return `@sprite.when_touching_object("_mouse_")`;
            }
            if (tgt === "_edge_") {
                return `@sprite.when_touching_object("_edge_")`;
            }
            return `@sprite.when_touching_object("${tgt.replace(/"/g, '\\"')}")`;
        }
        ...
        case "motion_glidesecstoxy": {
            ...
            const gx = f("X") || "0";
            const gy = f("Y") || "0";
            return `${ind}sprite.glide_secs_to_xy(${secs}, ${gx}, ${gy})`;
        }
        case "motion_glideto": {
            const secs = f("SECS") || "1";
            const gto = f("TO") || "";
            if (gto === "_random_") {
                return `${ind}sprite.glide_to("_random_", ${secs})`;
            }
            return `${ind}sprite.glide_to("${gto.replace(/"/g, '\\"')}", ${secs})`;
        }
        case "motion_glideto_menu": {
            const gto = f("TO") || "";
            if (gto === "_random_") return '"_random_"';
            if (gto === "_mouse_") return '"_mouse_"';
            return `"${gto.replace(/"/g, '\\"')}"`;
        }
        case "motion_pointtowards": {
            const towards = f("TOWARDS") || "";
            if (towards === "_mouse_") {
                return `${ind}sprite.point_towards("_mouse_")`;
            }
            return `${ind}sprite.point_towards("${towards.replace(/"/g, '\\"')}")`;
        }
        case "motion_pointtowards_menu": {
            const tw = f("TOWARDS") || "";
            if (tw === "_mouse_") return '"_mouse_"';
            return `"${tw.replace(/"/g, '\\"')}"`;
        }
        case "motion_setrotationstyle": {
            const style = f("STYLE") || "all around";
            return `${ind}sprite.set_rotation_style("${style.replace(/"/g, '\\"')}")`;
        }
        ...
        case "looks_gotofrontback": {
            const fb = f("FRONT_BACK") || "front";
            return `${ind}sprite.go_to_front_back("${fb}")`;
        }
        case "looks_goforwardbackwardlayers": {
            const num = f("NUM") || "1";
            if (f("FORWARD_BACKWARD") === "backward") {
                return `${ind}sprite.go_forward_backward_layers(-${num})`;
            }
            return `${ind}sprite.go_forward_backward_layers(${num})`;
        }
        case "looks_costumenumbername": {
            const nn = f("NUMBER_NAME") || "number";
            return `${ind}sprite.costume_number_name("${nn}")`;
        }
        case "looks_size":
