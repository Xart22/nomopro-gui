...
const messages = defineMessages({
    ...
    machineLearningTag: {
        id: 'gui.library.machineLearningTag',
...
const COMMUNICATION_TAG = {
    tag: 'communication',
    intlLabel: messages.communicationTag
};
const MACHINE_LEARNING_TAG = {
    tag: 'machinelearning',
...
class ExtensionLibrary extends React.PureComponent {
    constructor (props) {
        super(props);
        bindAll(this, ['updateDeviceExtensions', 'handleItemSelect']);
        this.state = {
            deviceExtensions: []
        };
    }
    ...
    handleItemSelect (item) {
        ...
        if (this.props.isRealtimeMode) {
            ...
            if (!item.disabled && !id) {
                ...
                url = prompt(
                    this.props.intl.formatMessage(messages.extensionUrl)
                );
            }
            if (id && !item.disabled) {
                const dispatchAddEvent = () => {
                    const match = extensionLibraryContent.find(ext => ext.extensionId === id);
                    const name = match && match.name && match.name.props && match.name.props.defaultMessage ?
                        match.name.props.defaultMessage :
                        id;
                    window.dispatchEvent(
                        new CustomEvent('python-ide-add-extension', {
                            detail: {name}
                        })
                    );
                };
            ...
            if (this.props.vm.extensionManager.isDeviceExtensionLoaded(id)) {
                this.props.vm.extensionManager
                    .unloadDeviceExtension(id)
                    .then(() => {
                        this.updateDeviceExtensions();
                    });
            } else {
                this.props.vm.extensionManager
                    .loadDeviceExtension(id)
                    .then(() => {
                        this.updateDeviceExtensions();
                        analytics.event({
                            category: 'extensions',
    ...
    render () {
        ...
        const device = this.props.deviceData.find(
            dev => dev.deviceId === this.props.deviceId
        );
        ...
        if (this.props.isRealtimeMode) {
            ...
            extensionLibraryThumbnailData = this.state.deviceExtensions
                .filter(
                    extension =>
                        extension.supportDevice.includes(this.props.deviceId) ||
                        extension.supportDevice.includes(
                            device.deviceExtensionsCompatible
                        )
                )
                .map(extension => ({
                    rawURL: extension.iconURL || extensionIcon,
...
export default compose(injectIntl, connect(mapStateToProps))(ExtensionLibrary);
