...
const PythonIde = props => {
    ...
    const supportsMicroPython = currentDevice ?
        (currentDevice.supportsMicroPython || currentDevice.tags?.includes('microPython')) :
        false;

    const isMicroPythonDevice = supportsMicroPython && (
    ...
    useEffect(() => {
        ...
        const handleSerialData = data => {
            ...
            if (window.__serialTerminalBuffer) {
                ...
                const combined = new Uint8Array(buf.byteLength + uint8.byteLength);
                combined.set(buf, 0);
                combined.set(uint8, buf.byteLength);
                if (combined.byteLength > 65536) {
                    window.__serialTerminalBuffer = combined.slice(combined.byteLength - 65536);
                } else {
    ...
    useEffect(() => {
        ...
        const handleStdout = data => {
            ...
            if (text) {
                const cleaned = text.replace(/\x1b\[[0-9;]*m/g, '').replace(/\r\n?/g, '')
                    .trim();
    ...
    const appendOutputLine = line => {
        ...
        setRunOutput(prev => {
            ...
            if (sprite) {
                return [
                    ...prev,
                    {type: 'line', spriteUrl: sprite.url, spriteName: sprite.name, text}
                ];
            }
    ...
    const queueVmCommand = command => {
        ...
        if (devId && vm && vm.runtime) {
            ...
            if (periph) {
                const ready = typeof periph.isReady === 'function' ? periph.isReady() : '?';
                const connected = typeof periph.isConnected === 'function' ? periph.isConnected() : '?';
                console.log('[QUEUE_CMD]', cmd, 'isReady=', ready, 'isConnected=', connected);
            } else {
    ...
    const createRunner = async () => {
        let candidates;
        if (pythonBackend !== 'pyodide' && typeof window !== 'undefined' && window.electronAPI?.getPythonCandidates) {
            try {
    ...
    const handleRun = async () => {
        ...
        try {
            ...
            if (result.exitCode !== 0) {
                appendOutputLine(`Process finished (exitCode=${result.exitCode}).`);
            }
        } catch (error) {
    ...
    const handleUploadToDevice = async code => {
        ...
        if (isMicrobit) {
            ...
            setIsUploading(true);
            setUploadProgress({stage: 'upload', percent: 0, text: 'Uploading...'});
    ...
    const getPeripheral = () => (vm && vm.runtime && vm.runtime.peripheralExtensions ? vm.runtime.peripheralExtensions[deviceId] : null);
    ...
    const handleDetectFirmware = () => {
        ...
        if (peripheral && typeof peripheral.micropythonUpload === 'function') {
            try {
                peripheral.micropythonUpload('', {detectOnly: true, board: deviceId});
            } catch (e) {
                setFirmwareStatus('error');
            }
            return;
    ...
    const handleFlashFirmware = () => {
        ...
        setUploadLog([]);
        setIsUploading(true);
        setUploadProgress({stage: 'flash', percent: 0, text: 'Starting flash...'});
    ...
    useEffect(() => {
        if (runtimeTarget === 'micropython' && isDeviceConnected && supportsMicroPython) {
    ...
    const handleUploadRun = () => {
        ...
        setUploadLog([]);
        const onSend = data => vm.writeToPeripheral(deviceId, data);
        micropythonRunnerRef.current.uploadMain(code, onSend, {shouldReset: true});
    };
    ...
    const handleUploadOnly = () => {
        ...
        setUploadLog([]);
        const onSend = data => vm.writeToPeripheral(deviceId, data);
        micropythonRunnerRef.current.uploadMain(code, onSend, {shouldReset: false});
    };
...
const mapDispatchToProps = dispatch => ({
    ...
    onSetModuleLibraryItems: items =>
        dispatch(setModuleLibraryItems(items)),
